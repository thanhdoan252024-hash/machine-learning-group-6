...
def main():
    ...
    plot_target_distribution(clean_df, str(FIG_DIR))
    plot_feature_scatter(clean_df, str(FIG_DIR))

    # Correlation heatmap
    plt.figure(figsize=(10, 8))
    corr = clean_df[[c for c in clean_df.columns if c.startswith('X') or c == 'Y1']].corr(numeric_only=True)
    sns.heatmap(corr, annot=False, cmap='coolwarm')
    plt.title('Feature Correlation Heatmap with Y1')
    plt.tight_layout()
