...
def main():
    ...
    parser.add_argument('--dataset-dir', type=Path, default=ROOT / 'UCI HAR Dataset')
    ...
    try:
        ...
        readme = '\n'.join([
            ...
            f'python scripts/verify_pca_artifacts.py --run-dir "{run_dir.relative_to(ROOT).as_posix() if run_dir.is_relative_to(ROOT) else run_dir}" --dataset-dir "UCI HAR Dataset"',
        ...
        subprocess.run([sys.executable, str(ROOT / 'scripts/verify_pca_artifacts.py'), '--run-dir', str(run_dir),
                        '--dataset-dir', str(dataset)], check=True)
        (ROOT / 'runs').mkdir(exist_ok=True)
        write_json(ROOT / 'runs/latest.json', {'run_dir': os.path.relpath(run_dir, ROOT).replace(os.sep, '/'),
                                             'status': 'completed', 'completed_at_utc': status['completed_at_utc']})
